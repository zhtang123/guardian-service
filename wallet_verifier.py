from get_events import EVMChainHandler
from get_transaction import Block, UserOp
import logging
import requests
import json
import os
...
class WalletVerifier:
    ...
    def __init__(self, database, max_block_number=0):
        self.events_handler = EVMChainHandler("chains.json")
        self.database = database
        self.max_block_number = max_block_number

    def verify(self, user_operation_hash, chain):

        logging.info("get transaction start")
        transaction_handler = UserOp(user_operation_hash, chain)
        ...
        ops = self.events_handler.get_transaction_events(chain, block.transaction_hash)
